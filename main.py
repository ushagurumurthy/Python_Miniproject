...
class Customer:   # Details of customer
  def __init__(self,A):
...
def createAccount(): # Creating Account
  try:
...
def ViewAllAccount(): #View the accounts created
  try:
    file=open('bank.bin','rb')
...
def Withdraw():   #Withdraw amount from the account
  file=open('bank.bin','rb')
  file1=open('tmp.bin','wb')
  x=int(input("Enter Bank Account: "))
  try:
...
def Update():  #Update account details
  file=open('bank.bin','rb')
  file1=open('tmp.bin','wb')
  x=int(input("Enter Bank Account: "))
  try:
...
def Exit():  #exit from the application
  pass

def option():
  print()
  print("Select your appropriate choice \n1. Create Account\n2. View all Account\n3. Deposit\n4. Withdraw\n5. Update\n6. Search\n7. Exit")
  n=int(input())
  if n==1:
    createAccount()
  elif n==2:
